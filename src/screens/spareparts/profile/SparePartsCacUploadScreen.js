...
const SparePartsCacUploadScreen = ({ navigation, route }) => {
  ...
  const handleUpload = () => {
    ...
    if (isOnboarding) {
      navigation.replace(ROUTES.SPARE_PARTS_UPLOAD_NIN, { onboarding: true });
      return;
    }
    ...
    <ScreenContainer padded={false} edges={['top', 'left', 'right', 'bottom']} style={styles.screen}>
      <View style={styles.container}>
        ...
        <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
          ...
          <TouchableOpacity
            style={[styles.uploadCard, cacImages.length ? styles.uploadCardFilled : null]}
            activeOpacity={0.9}
            onPress={addImage}
          >
            {cacImages.length ? (
              <Image source={{ uri: cacImages[0] }} style={styles.uploadedImage} />
            ) : (
              <>
                <View style={styles.uploadIconBadge}>
                  <HugeiconsIcon icon={ImageUploadIcon} size={24} color={darkTheme.colors.accent} strokeWidth={1.9} />
                </View>
                <AppText style={styles.uploadCardTitle}>Upload CAC</AppText>
                <AppText style={styles.uploadCardSubtitle}>
                  Add a clear photo of your CAC document for verification
                </AppText>
